...
export default function ScanPage() {
  ...
  return (
    <div className="px-4 py-4 md:px-6 md:py-6 space-y-4 md:space-y-6">
      <div>
        <h1 className="text-xl md:text-2xl font-bold text-foreground" data-testid="page-title-scan">Scan & Organize</h1>
            ...
            {results.map((result, i) => {
              ...
              return (
                <div
                  ...
                  className={`px-4 py-3 transition-colors ${accepted.has(i) ? "bg-green-50 dark:bg-green-900/10" : ""}`}
                >
                  <div className="flex items-start justify-between gap-2">
                    <div className="min-w-0 flex-1">
                      <div className="text-sm font-medium text-foreground truncate">{result.originalName}</div>
                      <div className="text-xs font-mono text-primary truncate mt-0.5">{result.suggestedName}</div>
                      <div className="text-xs text-muted-foreground mt-0.5">
                        {result.category}{result.subCategory ? ` / ${result.subCategory}` : ""}
                        {conf != null && <span className={`ml-2 font-semibold ${confColor}`}>{conf}%</span>}
                      </div>
